...
def test_scrublet_pickle_output(raw_batch_h5ad_path, temp_h5ad_file, tmp_path):
    ...
    assert "scrublet_predicted_doublet" in pickle_df.columns, (
        "Predicted doublets not found in pickle"
    )
    assert "scrublet_doublet_score" in pickle_df.columns, (
        "Doublet scores not found in pickle"
    )
    assert pickle_df.index.equals(adata.obs.index), (
    ...
    assert pickle_df["scrublet_doublet_score"].equals(adata.obs["doublet_score"]), (
